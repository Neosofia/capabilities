...
**In scope:** generic evaluation engine, namespace-scoped HTTP API, Cedar policy loading from an injected bundle, entitlement discovery from `View` permits on `ui::` entities.

**Out of scope:** product-specific menu or feature definitions (owned by the deploying product, e.g. CDP `policies/capabilities/ui/`), and complex request-scoped authorization (owned by each backend service's Cedar policies at the API boundary).

Product deployments inject a policy bundle at runtime (for example, CDP mounts `cdp/policies/capabilities/` locally, or production copies from the `cdp-policies` image at build time). The bundle is Cedar under `ui/` (see CDP `policies/capabilities/CONVENTIONS.md`). Response keys are Cedar entity ids (`ui::Menu::"clinician"`, etc.).
...
| `CAPABILITIES_POLICIES_DIR` | path | `policies` | Directory containing Cedar UI capability policies (`**/*.cedar`). |
